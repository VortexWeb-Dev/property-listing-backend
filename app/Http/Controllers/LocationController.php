...
public function bulkUploadLocations(Request $request)
{
    ...
    $requiredHeaders = ['City', 'Community', 'Sub Community', 'Building', 'Location', 'Type'];
    ...
    foreach ($csvData as $index => $row) {
        ...
        if (!empty($missingFields)) {
            return response()->json([
                'status' => 'error',
                'message' => "Row " . ($index + 2) . " is missing fields: " . implode(', ', $missingFields)
            ], 422);
        }
        ...
        if (!in_array($type, ['pf', 'bayut'])) {
            return response()->json([
                'status' => 'error',
                'message' => "Row " . ($index + 2) . ": 'Type' must be 'pf' or 'bayut'."
            ], 422);
        }
